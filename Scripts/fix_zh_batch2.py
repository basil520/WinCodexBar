...
fixes = [
    ...
    # OpenCode Go
    ("No OpenCode Go auth cookie found in browser cookies or manual input.", "未找到 OpenCode Go 认证 Cookie。请在浏览器 Cookie 或手动输入中检查。"),
    ...
    # ProviderSettings
    ("API Key", "API 密钥"),
    ("Manual cookie header", "手动 Cookie 头部"),
    ("Region", "区域"),
    ("Workspace ID", "工作区 ID"),
    ("Browser or manual cookie", "浏览器或手动 Cookie"),

    # QML strings (newly wrapped in qsTr)
...
for src, trans in fixes:
    ...
    if n > 0:
        ...
        # Try empty translation
        pattern2 = r'(<source>' + re.escape(src) + r'</source>\s*<translation>)\s*(</translation>)'
        replacement2 = r'\1' + trans + r'\2'
